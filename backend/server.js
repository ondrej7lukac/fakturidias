...
if (isProd && (!process.env.SESSION_SECRET || !process.env.SESSION_SECRET_2)) {
  console.error(
    "FATAL: SESSION_SECRET and SESSION_SECRET_2 must be set in production",
  );
  process.exit(1);
}
...
publicRouter.add("GET", "/health", ({ res }) =>
...
function serveStatic(req, res, requestPath) {
  ...
  if (
    !filePath.startsWith(path.resolve(distDir) + path.sep) &&
    filePath !== path.resolve(distDir)
  ) {
...
const handleRequest = async (req, res) => {
  ...
  logDebug("server", "incoming request", {
    method: req.method,
    path: requestPath,
  });
  ...
  if (requestPath.startsWith("/api/")) {
    ...
    if (
      !realIsAdmin &&
      !admin.isAdmin(effective) &&
      (await isUserSuspended(effective))
    ) {
      return sendJson(res, 403, {
...
if (require.main === module) {
  ...
  server.listen(port, "0.0.0.0", () =>
    console.log(
      `[server] Running at http://0.0.0.0:${port}/ (${isProd ? "production" : "development"})`,
    ),
  );
  startScheduler();
}
