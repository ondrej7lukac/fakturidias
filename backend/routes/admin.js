...
function isPlaceholder(v) {
  return (
    /^(your[_-]|placeholder|change[_-]?me|todo|example|dummy)/i.test(v) ||
    /x{3,}$/i.test(v) ||
    /\.{3,}$/.test(v) ||
    /^<.+>$/.test(v)
  );
...
async function readBody(ctx) {
  try {
    return await parseBody(ctx.req);
  } catch {
    sendJson(ctx.res, 400, { error: 'Invalid body' });
    return null;
  }
}
...
function attach(router) {
  ...
  router.add('PUT', '/api/admin/users/plan', async (ctx) => {
    ...
    if (ok) {
      await logAdminAction(adminEmail(ctx), 'plan_override', email, {
        plan,
  ...
  router.add('DELETE', '/api/admin/users/data', async (ctx) => {
    ...
    if (isAdmin(email))
      return sendJson(ctx.res, 400, { error: 'Cannot delete admin account' });
    const ok = await deleteUserAllData(email);
    if (ok)
      await logAdminAction(adminEmail(ctx), 'delete_user_data', email, {});
    return sendJson(ctx.res, ok ? 200 : 500, { success: ok });
  });
  ...
  router.add('POST', '/api/admin/promos', async (ctx) => {
    ...
    if (existing)
      return sendJson(ctx.res, 409, { error: 'Promo code already exists' });
    ...
    if (promo)
      await logAdminAction(adminEmail(ctx), 'promo_create', code, {
        planGrant: planGrant || null,
      });
    return sendJson(
      ctx.res,
      promo ? 201 : 500,
      promo ? { promo } : { error: 'Failed to create promo' },
    );
  });
  ...
  router.add('PUT', '/api/admin/promos', async (ctx) => {
    ...
    if (promo)
      await logAdminAction(adminEmail(ctx), 'promo_update', code, sanitized);
    return sendJson(
      ctx.res,
      promo ? 200 : 404,
      promo ? { promo } : { error: 'Promo not found' },
    );
  });
  ...
  router.add('POST', '/api/admin/promos/apply', async (ctx) => {
    ...
    if (!code || !userEmail)
      return sendJson(ctx.res, 400, { error: 'code and userEmail required' });
    const result = await applyPromoToUser(code, userEmail);
    if (result.error) return sendJson(ctx.res, 400, { error: result.error });
    await logAdminAction(adminEmail(ctx), 'promo_apply', userEmail, {
      code,
  ...
  router.add('GET', '/api/admin/user-detail', async (ctx) => {
    ...
    if (!detail)
      return sendJson(ctx.res, 503, {
        error: 'DB not connected or user not found',
      });
  ...
  router.add('GET', '/api/admin/health', async (ctx) => {
    ...
    const googleSecret = integrationStatus(process.env.GOOGLE_CLIENT_SECRET, {
      minLength: 10,
    });
    const resend = integrationStatus(process.env.RESEND_API_KEY, {
      minLength: 20,
      test: (v) => v.startsWith('re_'),
    });
    const inboundWebhook = integrationStatus(
      process.env.INBOUND_WEBHOOK_SECRET,
      {
        minLength: 20,
      },
    );
    const inboundDomain = integrationStatus(process.env.INBOUND_DOMAIN, {
      minLength: 3,
      test: (v) => v.includes('.'),
    });
    ...
    return sendJson(ctx.res, 200, {
      ...
      integrations: {
        stripe: integrationStatus(process.env.STRIPE_SECRET_KEY, {
          minLength: 30,
          test: (v) => /^(sk|rk)_(live|test)_/.test(v),
        }),
        stripeWebhook: integrationStatus(process.env.STRIPE_WEBHOOK_SECRET, {
          minLength: 20,
          test: (v) => v.startsWith('whsec_'),
        }),
        resend,
        inboundWebhook,
        inboundDomain,
        inboundMailbox: {
          ok: resend.ok && inboundWebhook.ok && inboundDomain.ok,
          note:
            resend.ok && inboundWebhook.ok && inboundDomain.ok
              ? 'Configured'
              : 'Missing inbound email setup',
        },
        googleOAuth: {
          ok: googleId.ok && googleSecret.ok,
          note:
            googleId.ok && googleSecret.ok
              ? 'Configured'
              : 'Missing ID or secret',
        },
        vertexAI: integrationStatus(process.env.VERTEX_PROJECT_ID, {
          minLength: 4,
        }),
      },
      runtime: {
  ...
  router.add('POST', '/api/admin/users/cancel-subscription', async (ctx) => {
    if (!guardSuper(ctx)) return true;
    if (!stripe)
      return sendJson(ctx.res, 503, { error: 'Stripe not configured' });
    const body = await readBody(ctx);
    if (!body) return true;
    ...
    if (!sub?.stripeSubscriptionId) {
      return sendJson(ctx.res, 400, {
        error: 'No Stripe subscription for this user',
      });
    }
  ...
  router.add('POST', '/api/admin/users/refund', async (ctx) => {
    ...
    try {
      ...
      if (charge.refunded)
        return sendJson(ctx.res, 400, {
          error: 'Most recent charge already refunded',
        });
  ...
  router.add('POST', '/api/admin/users/sync-stripe', async (ctx) => {
    ...
    try {
      ...
      await logAdminAction(adminEmail(ctx), 'subscription_sync', email, {
        status: stripeSub.status,
      });
  ...
  router.add('POST', '/api/admin/users/retry-payment', async (ctx) => {
    ...
    try {
      ...
      if (!inv)
        return sendJson(ctx.res, 400, { error: 'No open invoice to retry' });
  ...
  router.add('PUT', '/api/admin/settings', async (ctx) => {
    ...
    if (!updated)
      return sendJson(ctx.res, 500, { error: 'Failed to update settings' });
  ...
  router.add('POST', '/api/admin/email-templates', async (ctx) => {
    ...
    return sendJson(
      ctx.res,
      tpl ? 201 : 500,
      tpl ? { template: tpl } : { error: 'Failed to save template' },
    );
  });
  ...
  router.add('POST', '/api/admin/scheduled-emails', async (ctx) => {
    ...
    if (!subject || !message || !sendAt) {
      return sendJson(ctx.res, 400, {
        error: 'subject, message and sendAt required',
      });
    }
    const when = new Date(sendAt);
    if (isNaN(when.getTime()))
      return sendJson(ctx.res, 400, { error: 'Invalid sendAt' });
    const seg = ['all', 'free', 'standard', 'max', 'paid'].includes(segment)
      ? segment
      : 'all';
    ...
    return sendJson(
      ctx.res,
      scheduled ? 201 : 500,
      scheduled ? { scheduled } : { error: 'Failed to schedule' },
    );
  });
  ...
  router.add('POST', '/api/admin/webhooks/replay', async (ctx) => {
    ...
    if (!evt.payload)
      return sendJson(ctx.res, 400, { error: 'Event has no stored payload' });
    try {
      await processStripeEvent(evt.payload);
      await markWebhookEvent(body.id, 'replayed', null);
      await logAdminAction(adminEmail(ctx), 'webhook_replay', evt.type, {
        eventId: evt.eventId,
      });
      return sendJson(ctx.res, 200, { success: true });
    } catch (err) {
...
function attachPublic(router) {
  ...
  router.add('POST', '/api/webhooks/resend', async (ctx) => {
    ...
    if (!secret)
      return sendJson(ctx.res, 503, { error: 'Resend webhook not configured' });
    ...
    try {
      ...
      return sendJson(ctx.res, 400, { error: 'Invalid body' });
    }
    ...
    if (bounceType) {
      ...
      const reason =
        body.data?.bounce?.message || body.data?.reason || body.type;
