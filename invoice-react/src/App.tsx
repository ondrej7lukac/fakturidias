...
function App() {
  ...
  const [bankSync, setBankSync] = useState<{
    fioToken?: string;
    lastSyncAt?: string;
  } | null>(null);
    ...
    const loadSettings = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (settings?.bankSync) {
            setBankSync(settings.bankSync);
          }
        }
      } catch (err) {
  ...
  return (
    <>
      ...
      >
        {currentView === 'admin' ? (
          ...
          <Settings
            ...
            bankSync={bankSync}
            setBankSync={setBankSync}
            onInvoicesRefresh={fetchInvoices}
          />
        ) : (
          <>
            <div
      ...
      {impersonating && (
        ...
        >
          <span>
            Viewing as <strong>{impersonating}</strong> — changes affect their
            account.
          </span>
          <button
