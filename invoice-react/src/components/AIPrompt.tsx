...
function summarizeItems(
  data: any,
  vatPayer: boolean,
): { rows: PreviewLine[]; subtotal: number; tax: number; total: number } {
...
export default function AIPrompt({
  ...
  const { announce: onActivity } = useLiveActivity();
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          setError(
            isCz ? 'Chyba při čtení nahrávky.' : 'Failed to read recording.',
          );
          onActivity?.({
            kind: 'error',
            label: isCz ? 'Nepovedlo se' : 'Couldn’t read it',
          });
          return;
        }
        ...
        onActivity?.({
          kind: 'processing',
          label: isCz ? 'Zpracovávám hlas…' : 'Processing your voice…',
        });
        try {
          ...
          onActivity?.({
            kind: 'done',
            label: isCz ? 'Náhled připraven' : 'Preview ready',
          });
        } catch (err: any) {
          setError(
            err?.message ||
              (isCz
                ? 'Nepodařilo se zpracovat nahrávku.'
      ...
      onActivity?.({
        kind: 'listening',
        label: isCz ? 'Posloucháme…' : 'Listening…',
      });
    } catch (err: any) {
      const denied =
  ...
  const handleImageFile = async (file: File | undefined | null) => {
    ...
    if (file.type && !ALLOWED_TYPES.includes(file.type)) {
      setError(
        isCz ? 'Nepodporovaný formát obrázku.' : 'Unsupported image format.',
      );
      return;
    }
    ...
    onActivity?.({
      kind: 'scanning',
      label: isCz ? 'Skenuji fakturu…' : 'Scanning your invoice…',
    });
    try {
      const { base64, mimeType, dataUrl } = await resizeImage(file);
      setScanPreview({
        dataUrl,
        name: file.name || (isCz ? 'fotografie' : 'photo'),
      });
      ...
      setError(
        err?.message ||
          (isCz
            ? 'Nepodařilo se přečíst fakturu z obrázku.'
            : 'Failed to read invoice from image.'),
      );
      onActivity?.({
        kind: 'error',
        label: isCz ? 'Nepovedlo se' : 'Couldn’t read it',
  ...
  const processPrompt = async () => {
    ...
    onActivity?.({
      kind: 'processing',
      label: isCz ? 'Čtu vaše zadání…' : 'Reading your prompt…',
    });
    try {
      ...
      setError(
        err.message || (isCz ? 'Chyba při zpracování.' : 'Processing failed.'),
      );
      onActivity?.({
        kind: 'error',
        label: isCz ? 'Nepovedlo se' : 'Couldn’t read it',
  ...
  if (preview) {
    ...
    const itemsValue = `${itemCount} ${
      itemCount === 1 ? (isCz ? 'položka' : 'item') : isCz ? 'položek' : 'items'
    }`;
    const FieldIcon =
        ...
        <div className='lp-demo__card'>
          ...
          <div className='lp-demo__chips'>
            <div className='lp-demo__chip'>
              <div className='lp-demo__chip-label'>
                {isCz ? 'Klient' : 'Client'}
              </div>
            ...
            <div className='lp-demo__chip'>
              <div className='lp-demo__chip-label'>
                {isCz ? 'Bez DPH' : 'Subtotal'}
              </div>
              <div className='lp-demo__chip-value'>
                {money(summary.subtotal)} {currency}
              </div>
            </div>
          ...
          <div className='lp-demo__action'>
            <button
              ...
              onClick={editPreview}
            >
              <Pencil size={ICON_SM} strokeWidth={STROKE} />{' '}
              {isCz ? 'Upravit' : 'Edit'}
            </button>
            <button
              type='button'
            ...
            >
              ...
              {isCz ? 'Vytvořit fakturu' : 'Create invoice'}
            </button>
          </div>
        </div>
      </div>
    ...
    <div className='ap-ai'>
      <div className='ap-ai__head'>
        <h3>
          <Sparkles size={ICON_MD} strokeWidth={STROKE} />
          {isCz
            ? 'Vyplňte fakturu hlasem nebo textem'
            : 'Fill the invoice by voice or text'}
        </h3>
      ...
      <div className='ap-ai__divider'>
        {isCz ? 'nebo naskenujte fakturu' : 'or scan an invoice'}
      </div>
