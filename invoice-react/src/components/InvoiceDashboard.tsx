...
import {
  type AccountantDateBasis,
  type AccountantDocumentScope,
  type AccountantExportLang,
  type AccountantExportOptions,
  type AccountantTerritoryFilter,
  type AccountantExportVariant,
...
function getInvoiceYear(dateValue?: string): number | null {
...
function getClientCountryCode(invoice: Invoice): string {
  return String(invoice.client?.country || '')
    .trim()
    .toUpperCase();
}
...
function getTerritoryType(
  invoice: Invoice,
): 'domestic' | 'eu' | 'foreign' | 'unknown' {
  const country = getClientCountryCode(invoice);
...
function matchesTerritoryFilter(
  invoice: Invoice,
  territoryFilter: AccountantTerritoryFilter,
): boolean {
  if (territoryFilter === 'all') return true;
  return getTerritoryType(invoice) === territoryFilter;
}
  ...
  return (
    <>
      <svg className='ap-donut' viewBox='0 0 180 180'>
        {arcs.map((a, i) => (
          <path key={i} d={a.path} fill={a.color} opacity={0.92} />
        ))}
...
export default function InvoiceDashboard({
  ...
  const [exportVariant, setExportVariant] =
    useState<AccountantExportVariant>('detail');
  const [exportTerritoryFilter, setExportTerritoryFilter] =
    useState<AccountantTerritoryFilter>('all');
  const [exportDateBasis, setExportDateBasis] =
    useState<AccountantDateBasis>('taxableSupplyDate');
  const [exportDocumentScope, setExportDocumentScope] =
    useState<AccountantDocumentScope>('issued');
  const [exportLanguages, setExportLanguages] = useState<
    AccountantExportLang[]
  >(['cs', 'en']);
  ...
  const L = {
    ...
    sub: isCz
      ? 'Spravujte a sledujte všechny faktury'
      : 'Manage and track all your invoices',
    back: isCz ? 'Zpět' : 'Back',
    newInvoice: isCz ? 'Nová faktura' : 'New invoice',
    search: isCz
      ? 'Hledat (číslo, klient, oblast…)'
      : 'Search (number, client, area…)',
    ...
    exportYear: isCz ? 'Rok exportu' : 'Export year',
    exportPaymentFilter: isCz ? 'Filtr plateb' : 'Payment filter',
    exportVariant: isCz ? 'Typ exportu' : 'Export type',
    exportFilterAll: isCz ? 'Všechny' : 'All',
    exportFilterPaid: isCz ? 'Jen zaplacené' : 'Paid only',
    exportFilterUnpaid: isCz ? 'Jen nezaplacené' : 'Unpaid only',
    exportVariantDetail: isCz ? 'Detail faktur' : 'Invoice detail',
    exportVariantMonthly: isCz ? 'Souhrn po měsících' : 'Monthly summary',
    exportVariantVat: isCz ? 'Souhrn podle DPH' : 'VAT summary',
    exportVariantSmallBusiness: isCz ? 'Malý podnikatel' : 'Small business',
    exportTerritory: isCz ? 'Územní filtr' : 'Client territory',
    exportTerritoryAll: isCz ? 'Všechny země' : 'All territories',
    exportTerritoryDomestic: isCz ? 'Tuzemsko (CZ)' : 'Domestic (CZ)',
    exportTerritoryEu: isCz ? 'EU mimo CZ' : 'EU outside CZ',
    exportTerritoryForeign: isCz ? 'Mimo EU' : 'Non-EU',
    exportDateBasis: isCz ? 'Rozhodné datum' : 'Filing date basis',
    exportDateBasisIssue: isCz ? 'Datum vystavení' : 'Issue date',
    exportDateBasisTaxable: isCz
      ? 'DUZP / datum zdanitelného plnění'
      : 'Taxable supply date',
    exportDocumentScope: isCz ? 'Doklady zahrnout' : 'Documents to include',
    exportDocumentScopeIssued: isCz
      ? 'Jen vystavené doklady'
      : 'Issued invoices only',
    exportDocumentScopeAll: isCz ? 'Včetně rozepsaných' : 'Include drafts too',
    exportLanguages: isCz ? 'Jazyky v ZIPu' : 'Languages in ZIP',
    exportLanguageCz: isCz ? 'Čeština' : 'Czech',
    exportLanguageEn: isCz ? 'Angličtina' : 'English',
    exportOpenModal: isCz ? 'Roční export' : 'Annual export',
    exportModalTitle: isCz
      ? 'Nastavení ročního exportu'
      : 'Annual export options',
    exportModalSub: isCz
      ? 'Vyberte typ exportu, územní filtr, rozhodné datum a jazyky. Export přidá i souhrnný přehled pro roční podklady.'
      : 'Choose the export type, territory filter, filing date basis, and languages. The export also includes a compact annual filing summary.',
    exportSelectedCount: isCz ? 'Vybraných faktur' : 'Matching invoices',
    exportSummaryIncluded: isCz
      ? 'Součástí ZIPu je i souhrnný přehled.'
      : 'A filing summary is included in the ZIP.',
    exportCancel: isCz ? 'Zrušit' : 'Cancel',
    exportSubmit: isCz ? 'Exportovat ZIP' : 'Export ZIP',
    exportNoLanguage: isCz
      ? 'Vyberte alespoň jeden jazyk exportu.'
      : 'Select at least one export language.',
    exportAnnualTax: isCz ? 'Účetní export ZIP' : 'Accountant ZIP export',
    exportAnnualTaxHint: isCz
      ? 'Vytvoří ZIP se dvěma CSV soubory pro účetní workflow: česky a anglicky, podle zvoleného filtru a typu exportu.'
      : 'Creates a ZIP with two CSV files for accountant workflow: Czech and English, using the selected filter and export type.',
    exportError: isCz
  ...
  const availableYears = useMemo(() => {
    const years = Array.from(
      new Set(
        invoices
          .flatMap((invoice) => [
            getInvoiceYear(invoice.issueDate),
            getInvoiceYear(invoice.taxableSupplyDate),
          ])
  ...
  useEffect(() => {
    if (!availableYears.includes(Number(exportYear))) {
      setExportYear(String(availableYears[0]));
    }
  }, [availableYears, exportYear]);

  const exportableInvoiceCount = useMemo(() => {
    const year = Number(exportYear);
    return invoices.filter((invoice) => {
      const dateValue =
        exportDateBasis === 'taxableSupplyDate'
          ? invoice.taxableSupplyDate || invoice.issueDate
          : invoice.issueDate;
      if (getInvoiceYear(dateValue) !== year) return false;
      if (
        exportDocumentScope === 'issued' &&
        !isIssuedInvoice(invoice.status)
      ) {
        return false;
      }
      if (!matchesTerritoryFilter(invoice, exportTerritoryFilter)) {
        return false;
      }
      if (exportPaymentFilter === 'paid') return invoice.status === 'paid';
      if (exportPaymentFilter === 'unpaid') return invoice.status !== 'paid';
      return true;
    }).length;
  }, [
    exportDateBasis,
    exportDocumentScope,
    exportPaymentFilter,
    exportTerritoryFilter,
    exportYear,
    invoices,
  ]);

  const exportOptions: AccountantExportOptions = {
    paymentFilter: exportPaymentFilter,
    variant: exportVariant,
    languages: exportLanguages,
    territoryFilter: exportTerritoryFilter,
    dateBasis: exportDateBasis,
    documentScope: exportDocumentScope,
  };

  const toggleExportLanguage = (language: AccountantExportLang) => {
    setExportLanguages((currentLanguages) =>
  ...
  const handleExportAnnualTax = async () => {
    if (!isAuthenticated) return;
    if (exportLanguages.length === 0) {
      window.alert(L.exportNoLanguage);
      return;
    }
    setIsExporting(true);
    try {
  ...
  const metrics = useMemo(() => {
    ...
    const prevRevenue = lastMonth.reduce(
      (s, inv) => s + (Number(inv.amount) || 0),
      0,
    );
    const revDeltaPct =
  ...
  const SortIcon = ({ k }: { k: string }) => {
    if (sortKey !== k)
      return (
        <span style={{ opacity: 0.3, marginLeft: 3, fontSize: 10 }}>↕</span>
      );
    return (
    ...
    <div className='ap-page'>
      ...
      {isAuthenticated && (
        <Dialog open={isExportModalOpen} onOpenChange={setIsExportModalOpen}>
          <DialogContent
            overlayClassName='invoice-export-modal-overlay z-[100000]'
            className='invoice-export-modal z-[100001]'
          >
            <DialogHeader className='invoice-export-modal__header'>
              <div>
                <div className='invoice-export-modal__eyebrow'>
                  {L.exportOpenModal}
                </div>
                <DialogTitle className='invoice-export-modal__title'>
                  {L.exportModalTitle}
                </DialogTitle>
                <DialogDescription className='invoice-export-modal__sub'>
                  {L.exportModalSub}
                </DialogDescription>
              </div>
              <DialogClose
                className='ap-btn ap-btn--icon ap-btn--ghost'
                aria-label={L.exportCancel}
            ...
            <div className='invoice-export-modal__grid'>
              ...
              <div className='invoice-export-modal__field invoice-export-modal__field--full'>
                <span>{L.exportLanguages}</span>
                <div className='invoice-export-modal__checks'>
                  <label className='invoice-export-modal__check'>
                    <input
                      type='checkbox'
                      checked={exportLanguages.includes('cs')}
                      onChange={() => toggleExportLanguage('cs')}
                    />
                    <span>{L.exportLanguageCz}</span>
                  </label>
                  <label className='invoice-export-modal__check'>
                    <input
                      type='checkbox'
                      checked={exportLanguages.includes('en')}
                      onChange={() => toggleExportLanguage('en')}
                    />
                    <span>{L.exportLanguageEn}</span>
                  </label>
                </div>
              </div>
            </div>
            ...
            <div className='invoice-export-modal__summary'>
              <span className='invoice-export-modal__summary-copy'>
                {L.exportSelectedCount}
                <br />
                <small>{L.exportSummaryIncluded}</small>
              </span>
              <strong className='invoice-export-modal__summary-value'>
                {exportableInvoiceCount}
              </strong>
            </div>

            <div className='invoice-export-modal__actions'>
              <button
      ...
      <div className='ap-metrics'>
        <div className='ap-metric'>
          ...
          {metrics.revDeltaPct !== null ? (
            <div
              className={`ap-metric__delta ${metrics.revDeltaPct >= 0 ? 'ap-metric__delta--up' : 'ap-metric__delta--down'}`}
            >
              ...
              {metrics.revDeltaPct >= 0 ? '+' : ''}
              {metrics.revDeltaPct.toFixed(1)} %
            </div>
          ) : (
            <div className='ap-metric__delta' style={{ color: 'var(--muted)' }}>
              <ArrowLeftRight size={11} strokeWidth={STROKE} />{' '}
              {isCz ? 'tento měsíc' : 'this month'}
            </div>
          )}
        </div>
        <div className='ap-metric'>
          ...
          <div className='ap-metric__delta ap-metric__delta--up'>
            <ArrowUp size={11} strokeWidth={STROKE} /> +{metrics.thisMonthCount}{' '}
            {isCz ? 'tento měsíc' : 'this month'}
          </div>
        </div>
        <div
          className={`ap-metric${metrics.overdueCount > 0 ? ' ap-metric--danger' : ''}`}
        >
          ...
          {metrics.overdueCount > 0 ? (
            <div className='ap-metric__delta ap-metric__delta--down'>
              <ArrowDown size={11} strokeWidth={STROKE} />{' '}
              {isCz ? 'vyžaduje akci' : 'requires action'}
            </div>
          ) : (
            <div className='ap-metric__delta ap-metric__delta--up'>
              <ArrowUp size={11} strokeWidth={STROKE} />{' '}
              {isCz ? 'vše v pořádku' : 'all clear'}
            </div>
          )}
        </div>
        <div className='ap-metric'>
      ...
      <div style={{ marginBottom: 14, fontSize: 13, color: 'var(--muted)' }}>
        ...
        <strong style={{ color: 'var(--text)' }}>
          {totalValue.toLocaleString(isCz ? 'cs-CZ' : 'en-US', {
                    ...
                    <div className='expanded-row'>
                      ...
                      {inlineEditId === inv.id ? (
                        ...
                        <div className='expanded-row-grid'>
                          ...
                          <div className='expanded-section'>
                            <div className='expanded-section-title'>
                              {isCz ? 'Datum & Částka' : 'Dates & Amount'}
                            </div>
                            <div
                              style={{
                                fontSize: '0.85rem',
