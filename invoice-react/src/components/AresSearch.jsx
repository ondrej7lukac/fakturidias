...
export default function AresSearch({
  ...
  const [status, setStatus] = useState(
    t.aresPlaceholder || 'Zadejte název firmy nebo IČO...',
  );
  ...
  const applyAresEntity = (entity) => {
    ...
    const address =
      entity.adresa || entity.textovaAdresa || formatAresAddress(entity.sidlo);
            ...
            <div>
              ...
              <div className='inline-actions'>
                ...
                <Button
                  ...
                  size='sm'
                  variant='secondary'
                  onClick={() => lookupByIco(clientIco)}
                  className='ares-search-btn'
                >
                  ARES
                </Button>
              </div>
          ...
          {showResults && (
            <div className='suggestions'>
              {results.map((item, index) => (
                ...
                >
                  <strong>
                    {item.obchodniJmeno || item.nazev || 'Unknown'}
                  </strong>
                  <br />
                  <span className='invoice-meta'>
                    IČO {item.ico || 'n/a'} •{' '}
                    {item.adresa ||
                      formatAresAddress(item.sidlo) ||
                      'No address'}
                  </span>
                </button>
              ))}
