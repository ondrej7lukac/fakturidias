...
import { createMockInvoices } from './utils/mockInvoices';
...
function App() {
    ...
    if (defaultSupplier) {
      ...
      if (user) {
        ...
        fetch('/api/settings', {
          ...
          console.error('[Settings] Failed to save to MongoDB:', err),
        );
      } else {
        // Guest: save to localStorage only
        localStorage.setItem(
          'defaultSupplier_guest',
  ...
  const handleNewInvoice = () => {
    ...
    setDashboardOpen(false); // Ensure dashboard overlay closes
    setMobileView('form'); // Ensure form is visible on mobile
    setInvoicesLoaded(true); // Keep number generation path unlocked
  ...
  const handleLoadDemoInvoices = () => {
    ...
    const demoInvoices = createMockInvoices();
    replaceLocalInvoices(demoInvoices);
    setInvoices(demoInvoices);
    setSelectedId(demoInvoices[0]?.id ?? null);
    setCategories(
      Array.from(
        new Set(
          demoInvoices.map((invoice) => invoice.category).filter(Boolean),
        ),
      ).sort(),
    );
    setInvoiceCounter(getNextInvoiceCounter(demoInvoices));
    setDraftNumber('');
    ...
    <>
      ...
      <main className={dashboardOpen ? 'dashboard-mode' : ''}>
        {currentView === 'settings' ? (
          ...
          <>
            <div
              className={`invoice-form-container ${mobileView !== 'form' ? 'mobile-hidden' : ''}`}
            >
              <InvoiceForm
            ...
            <div
              className={`invoice-list-container ${mobileView !== 'list' ? 'mobile-hidden' : ''}`}
            >
              {isLoading ? (
                ...
                <InvoiceList
                  invoices={invoices}
                  categories={categories}
                  onLoadDemoInvoices={handleLoadDemoInvoices}
                  onClearDemoInvoices={handleClearDemoInvoices}
